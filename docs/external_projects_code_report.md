...
| **WebGL Particle Rasterizer Shader** (`src/components/SplatViewport3D.tsx`) | `m-schuetz/WebGL_GaussianSplatting` (`awesome-3D-gaussian-splatting`) | Custom WebGL GLSL shader rendering 3D Gaussian particles with soft radial alpha falloff $\exp(-\frac{1}{2} (x^2 + y^2))$ and depth sorting. |
